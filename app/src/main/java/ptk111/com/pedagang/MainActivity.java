...
import ptk111.com.pedagang.Database.LokasiUser;
...
public class MainActivity extends AppCompatActivity implements
    ...
    ArrayList<Marker> locationPembeli = new ArrayList<Marker>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //myToolbar.setTitle("Pedagang User");
    ...
    private void markerPembeli(LatLng latLng) {
        Log.i(TAG, "markerPembeli("+latLng+")");
        String title = latLng.latitude + ", " + latLng.longitude;
        // Define marker options
        MarkerOptions markerOptions = new MarkerOptions()
                .position(latLng)
                .icon(BitmapDescriptorFactory.fromResource(R.drawable.pembeli))
                //.icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_BLUE))
                .title(title);

        //locationPembeli = map.addMarker(markerOptions);
        locationPembeli.add(map.addMarker(markerOptions));
    }

    private void updatePembeli(LatLng latLng, int i) {
        Log.i(TAG, "updatePembeli("+latLng+")");
        String title = latLng.latitude + ", " + latLng.longitude;
        //locationPembeli.setPosition(latLng);
        locationPembeli.get(i).setPosition(latLng);
    }
    /* ----------------------------------------------------------------------------------- */
    ...
    Runnable mHandlerTask = new Runnable()
    {
        @Override
        public void run() {
            new AmbilData().execute( "http://agungcahya.esy.es/server.php?operasi=viewPembeli" );
            //stopRepeatingTask();
            startRepeatingTask2();
    ...
    Runnable mHandlerTask2 = new Runnable()
    {
        @Override
        public void run() {
            new updatePembeli().execute( "http://agungcahya.esy.es/server.php?operasi=viewPembeli" );
    ...
    private class AmbilData extends AsyncTask<String, Integer, String> {
        ...
        protected void onPostExecute(String result) {
            parseLokasiPembeli(result);
        }

    }

    private class updatePembeli extends AsyncTask<String, Integer, String> {
        protected String doInBackground(String... strUrl) {
            Log. v ( "yw" , "mulai ambil data" );
            String hasil= "" ;
            //ambil data dari internet
    ...
    private void parseLokasiPembeli(String hasil){
        ...
        try {
            ...
            for (int i = 0; i < arrayUser.length(); i++) {
                ...
                markerPembeli(new LatLng(listLatitude[i], listLongitude[i]));
            }
    ...
    private void updateLokasiPembeli(String hasil){
        ...
        try {
            ...
            for (int i = 0; i < arrayUser.length(); i++) {
                ...
                updatePembeli(new LatLng(listLatitude[i], listLongitude[i]), i);
            }
        }catch (JSONException e) {
            e.printStackTrace();
        }
